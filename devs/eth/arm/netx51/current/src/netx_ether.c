...
#include <pkgconf/devs_eth_arm_netx51.h>
...
#include <string.h>   //memcpy, memset
  ...
  #include <xpec_eth_std_mac_rpec0.h>
  #include <rpu_eth0.h>
  #include <tpu_eth0.h>
#endif

#if defined(CYGPKG_DEVS_ETH_ARM_NETX_ETH1)
  #include <xpec_eth_std_mac_rpec1.h>
  #include <rpu_eth1.h>
  #include <tpu_eth1.h>
#endif
...
static volatile cyg_uint32*        s_pulxPECInterruptRegisters = (volatile cyg_uint32*)Addr_NX51_xpec_irq_registers;
static NX51_POINTER_FIFO_AREA_T*   s_ptFifoArea                = (NX51_POINTER_FIFO_AREA_T*)Addr_NX51_pointer_fifo;
static bool                        s_fPhyInitialized           = false;
static cyg_uint8                   s_abPhyAddresses[]          =
{
  PHY_PORT0_ADDRESS,
  PHY_PORT1_ADDRESS
};
  ...
  {  0,    
     ...
     (cyg_uint32*)XcCode_xpec_eth_std_mac_rpec0, (cyg_uint32*)XcCode_rpu_eth0, (cyg_uint32*)XcCode_tpu_eth0, 
     Addr_NX51_rpec0_dram, NX51_NETX_XM0_BASE, NX51_NETX_MEM_INTRN_XC0_AREA,
     (volatile cyg_uint32*)Addr_NX51_rpec0_dram, (volatile cyg_uint32*)NX51_NETX_XM0_BASE, (volatile cyg_uint32*)NX51_NETX_MEM_INTRN_XC0_AREA,
  ...
  static NETX_ETH_PORT_INFO s_tPortInfo1 =
  {
     1,    CYGNUM_HAL_INTERRUPT_COM1, 
     (cyg_uint32*)XcCode_xpec_eth_std_mac_rpec1, (cyg_uint32*)XcCode_rpu_eth1, (cyg_uint32*)XcCode_tpu_eth1, 
     Addr_NX51_rpec1_dram, NX51_NETX_XM1_BASE, NX51_NETX_MEM_INTRN_XC1_AREA,
     (volatile cyg_uint32*)Addr_NX51_rpec1_dram, (volatile cyg_uint32*)NX51_NETX_XM1_BASE, (volatile cyg_uint32*)NX51_NETX_MEM_INTRN_XC1_AREA,
     {0, 0x11, 0x22, 0x33, 0x44, 0x56}
  };

  ETH_DRV_SC(netxeth1_sc,
             &s_tPortInfo1,     // No driver specific data needed
...
{
  NX51_MIIMU_AREA_T* ptMiiMu;
  NX51_MIIMU_T tVlu     = {0};

  if(PHY_PORT0_ADDRESS == uPhyPrt)
    ptMiiMu = (NX51_MIIMU_AREA_T*)Addr_NX51_int_phy_ctrl0;
  else if(PHY_PORT1_ADDRESS == uPhyPrt)
    ptMiiMu = (NX51_MIIMU_AREA_T*)Addr_NX51_int_phy_ctrl1;
  else
    ptMiiMu = (NX51_MIIMU_AREA_T*)Addr_NX51_miimu;

  /* set mode with preamble and phyaddr */
  tVlu.bf.snrdy = 1;
  tVlu.bf.preamble = 1; /* Send the Preamble with the Code */
  tVlu.bf.rta = 0;
  tVlu.bf.phyaddr = uPhyPrt;
  tVlu.bf.regaddr = uRegAdr;
  
  if(uAcc == 0) {
    tVlu.bf.opmode = 0;
    tVlu.bf.data = 0;
    ptMiiMu->ulMiimu = tVlu.val; /* Write the Command now to the PHY */

    do{
      tVlu.val = ptMiiMu->ulMiimu;
    } while(tVlu.bf.snrdy == 1); /* Wait until PHY is ready first of all */

    tVlu.val = ptMiiMu->ulMiimu;
    *puDat = (unsigned int) tVlu.bf.data; /* Read now the wished PHY data */

  } else if(uAcc == 1) {
    tVlu.bf.opmode = 1;
    tVlu.bf.data = (unsigned int) *puDat;
    ptMiiMu->ulMiimu = tVlu.val;/* Write the Command again now to the PHY */
...
static void EnableAsicControl(void)
{
  ...
  pulAsicCtrlKey = (volatile cyg_uint32*)Adr_NX51_asic_ctrl_access_key;
...
static bool InitInternalPHY(void)
{
  ...
  volatile unsigned long* pulPhyCtrl  = (volatile unsigned long*)Adr_NX51_phy_control;
  ...
  ulCfgValue = ((0xE << SRT_NX51_phy_control_phy_address)    & MSK_NX51_phy_control_phy_address)   | //set phy's to MDIO Addres 1C, 1D
               ((0x7 << SRT_NX51_phy_control_phy0_mode)      & MSK_NX51_phy_control_phy0_mode)     | //Auto negotitation, AutoMDIX
               ((0x1 << SRT_NX51_phy_control_phy0_automdix)  & MSK_NX51_phy_control_phy0_automdix) | //AutoMDIX
               ((0x1 << SRT_NX51_phy_control_phy0_enable)    & MSK_NX51_phy_control_phy0_enable)   | //enable phy 1
               ((0x7 << SRT_NX51_phy_control_phy1_mode)      & MSK_NX51_phy_control_phy1_mode)     | //Auto negotitation, AutoMDIX
               ((0x1 << SRT_NX51_phy_control_phy1_automdix)  & MSK_NX51_phy_control_phy1_automdix) | //AutoMDIX
               ((0x1 << SRT_NX51_phy_control_phy1_enable)    & MSK_NX51_phy_control_phy1_enable);    //enable phy 2

  EnableAsicControl();
  
  //reset PHY's
  *pulPhyCtrl = ulCfgValue | MSK_NX51_phy_control_phy_reset;
...
void netxeth_stop(struct eth_drv_sc *sc)
{
  ...
  ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_INTERRUPTS_ENABLE_IND_HI / sizeof(cyg_uint32)] = 0;
  ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_INTERRUPTS_ENABLE_IND_LO / sizeof(cyg_uint32)] = 0;
  ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_INTERRUPTS_ENABLE_CON_HI / sizeof(cyg_uint32)] = 0;
  ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_INTERRUPTS_ENABLE_CON_LO / sizeof(cyg_uint32)] = 0;
...
int netxeth_control(struct eth_drv_sc *sc, unsigned long cmd, void *data, int data_length)
{
  ...
  case ETH_DRV_GET_IF_STATS:
  {
    ...
    cyg_uint32*             pulxPECRamStart = (cyg_uint32*)&ptPortInfo->pulRPECDram[0];
...
*/
  ...
  case ETH_DRV_GET_MAC_ADDRESS: 
  {
    ...
    {
      cyg_uint16* pusMAC = (cyg_uint16*)data;
      pusMAC[2] = ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_HI / sizeof(cyg_uint32)];
      pusMAC[1] = ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_LO / sizeof(cyg_uint32)] >> 16;
      pusMAC[0] = ptPortInfo->pulRPECDram[REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_LO / sizeof(cyg_uint32)];
    }
  }
#endif  
  default:
...
void netxeth_deliver(struct eth_drv_sc *sc)
{
  ...
  cyg_uint32 ulInterrupts        = s_pulxPECInterruptRegisters[ptPortInfo->ulPort] & MSK_NX51_irq_xpec0_rtpec0_irq;
...
bool netxeth_init(struct cyg_netdevtab_entry *tab)
{
  ...
  if(s_pulxPECInterruptRegisters &&
     ptPortInfo->pulRPECDram &&
