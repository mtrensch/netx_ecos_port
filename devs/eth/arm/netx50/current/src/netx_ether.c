...
#include <pkgconf/devs_eth_arm_netx50.h>
...
static volatile cyg_uint32*  s_pulxPECInterruptRegisters = (volatile cyg_uint32*)Addr_xpec_irq_registers;
static volatile MIIMU_REG_T* s_ptMiiMu                   = (volatile MIIMU_REG_T*)Adr_miimu_reg;
static PFIFO_AREA_T          s_ptFifoArea                = (PFIFO_AREA_T)Addr_pointer_fifo;
...
static cyg_uint8             s_abPhyAddresses[]          =
{
  ...
  {  0,    
     ...
     Addr_xpec0, Addr_xmac0, Adr_intram0_base,
     (volatile cyg_uint32*)Addr_xpec0, (volatile cyg_uint32*)Addr_xmac0, (volatile cyg_uint32*)Adr_intram0_base,
  ...
  static NETX_ETH_PORT_INFO s_tPortInfo1 =
  {
     ...
     (cyg_uint32*)XcCode_xpec_eth_std_mac1, (cyg_uint32*)XcCode_rpu_eth1, (cyg_uint32*)XcCode_tpu_eth1, 
     Addr_xpec1, Addr_xmac1, Adr_intram1_base,
     (volatile cyg_uint32*)Addr_xpec1, (volatile cyg_uint32*)Addr_xmac1, (volatile cyg_uint32*)Adr_intram1_base,
...
static void EnableAsicControl(void)
{
  ...
  pulAsicCtrlKey = (volatile cyg_uint32*)Adr_asic_ctrl_access_key;
...
void netxeth_start(struct eth_drv_sc *sc, unsigned char *enaddr, int flags)
{
  ...
  // local mac config
  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_TRAFFIC_CLASS_ARRANGEMENT) / sizeof(cyg_uint32)] = (0x8 << SRT_ETHMAC_TRAFFIC_CLASS_ARRANGEMENT_VAL);
  ...
  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_INTERRUPTS_ENABLE_IND_HI) / sizeof(cyg_uint32)] = MSK_ETHMAC_INTERRUPTS_ENABLE_IND_HI_VAL;
  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_INTERRUPTS_ENABLE_IND_LO) / sizeof(cyg_uint32)] = MSK_ETHMAC_INTERRUPTS_ENABLE_IND_LO_VAL;
  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_INTERRUPTS_ENABLE_CON_HI) / sizeof(cyg_uint32)] = MSK_ETHMAC_INTERRUPTS_ENABLE_CON_HI_VAL;
  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_INTERRUPTS_ENABLE_CON_LO) / sizeof(cyg_uint32)] = MSK_ETHMAC_INTERRUPTS_ENABLE_CON_LO_VAL;

  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_HI) / sizeof(cyg_uint32)] = pusMAC[2];
  ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_LO) / sizeof(cyg_uint32)] = (pusMAC[1] << 16) |
                                                                                                            pusMAC[0];
...
int netxeth_control(struct eth_drv_sc *sc, unsigned long cmd, void *data, int data_length)
{
  ...
  case ETH_DRV_GET_IF_STATS:
  {
    ...
    cyg_uint32*             pulxPECRamStart = (cyg_uint32*)&ptPortInfo->pulxPECBase[REL_Adr_dram_start / sizeof(cyg_uint32)];
...
*/
  ...
  case ETH_DRV_GET_MAC_ADDRESS: 
  {
    ...
    {
      cyg_uint16* pusMAC = (cyg_uint16*)data;
      pusMAC[2] = ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_HI) / sizeof(cyg_uint32)];
      pusMAC[1] = ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_LO) / sizeof(cyg_uint32)] >> 16;
      pusMAC[0] = ptPortInfo->pulxPECBase[(REL_Adr_dram_start + REL_Adr_ETHMAC_LOCAL_MAC_ADDRESS_LO) / sizeof(cyg_uint32)];
    }
  }
#endif  
  default:
...
void netxeth_deliver(struct eth_drv_sc *sc)
{
  ...
  cyg_uint32 ulRecvFramesHi      = s_ptFifoArea->aulPFifoFillLevelBase[ulXCFifoOffset + IND_FIFO_PORT_HI];
  cyg_uint32 ulRecvFramesLo      = s_ptFifoArea->aulPFifoFillLevelBase[ulXCFifoOffset + IND_FIFO_PORT_LO];
  cyg_uint32 ulConfirmationsHi   = s_ptFifoArea->aulPFifoFillLevelBase[ulXCFifoOffset + CON_FIFO_PORT_HI];
  cyg_uint32 ulConfirmationsLo   = s_ptFifoArea->aulPFifoFillLevelBase[ulXCFifoOffset + CON_FIFO_PORT_LO];
  cyg_uint32 ulInterrupts        = s_pulxPECInterruptRegisters[ptPortInfo->ulPort] & MSK_irq_xpec0_xpec_irq;
