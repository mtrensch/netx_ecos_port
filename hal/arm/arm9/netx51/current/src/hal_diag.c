...
#define RX_DATA(s)		    (((s) & MSK_NX51_uartfr_RXFE) == 0)
#define TX_READY(s)		    (((s) & MSK_NX51_uartfr_TXFF) == 0)
...
static channel_data_t s_atSerialChannels[] = {
    { (volatile cyg_uint32*)NX51_NETX_UART0_AREA, 1000, CYGNUM_HAL_INTERRUPT_UART0 },
    { (volatile cyg_uint32*)NX51_NETX_UART1_AREA, 1000, CYGNUM_HAL_INTERRUPT_UART1 },
};
...
static void cyg_hal_plf_serial_init_channel(void* __ch_data)
{
  ...
  pulBase[REL_Adr_NX51_uartcr / sizeof(cyg_uint32)] = 0;

  // Set baud rate CYGNUM_HAL_VIRTUAL_VECTOR_CONSOLE_CHANNEL_BAUD
  pulBase[REL_Adr_NX51_uartlcr_m / sizeof(cyg_uint32)] = (ARM_NETX_BAUD_DIVISOR & 0xFF00) >> 8;
  pulBase[REL_Adr_NX51_uartlcr_l / sizeof(cyg_uint32)] = ARM_NETX_BAUD_DIVISOR & 0xFF;

  // ----------v----------v----------v----------v----------
  // NOTE: MUST BE WRITTEN LAST (AFTER UARTLCR_M & UARTLCR_L)
  // ----------^----------^----------^----------^----------
  // set the UART to be 8 bits, 1 stop bit, no parity, fifo enabled
  pulBase[REL_Adr_NX51_uartcr_2 / sizeof(cyg_uint32)]  = MSK_NX51_uartcr_2_Baud_Rate_Mode;
  pulBase[REL_Adr_NX51_uartlcr_h / sizeof(cyg_uint32)] = MSK_NX51_uartlcr_h_WLEN | MSK_NX51_uartlcr_h_FEN;

  pulBase[REL_Adr_NX51_uartdrvout / sizeof(cyg_uint32)] = MSK_NX51_uartdrvout_DRVTX; /* Set TX-Driver to enabled */

  // finally, enable the uart
  pulBase[REL_Adr_NX51_uartcr / sizeof(cyg_uint32)] = MSK_NX51_uartcr_uartEN;
}
...
void cyg_hal_plf_serial_putc(void *__ch_data, char c)
{
  ...
  do {
    status = pulBase[REL_Adr_NX51_uartfr / sizeof(cyg_uint32)];
  } while (!TX_READY(status));	// wait until ready

  pulBase[REL_Adr_NX51_uartdr / sizeof(cyg_uint32)] = c;
  ...
  if (c == '\n') 
  {
    ...
    pulBase[REL_Adr_NX51_uartdr / sizeof(cyg_uint32)] = '\r';
  }
...
static cyg_bool cyg_hal_plf_serial_getc_nonblock(void* __ch_data, cyg_uint8* ch)
{
  ...
  *ch = (cyg_uint8)(pulBase[REL_Adr_NX51_uartdr / sizeof(cyg_uint32)] & 0xFF);
...
cyg_hal_plf_serial_control(void* __ch_data, __comm_control_cmd_t __func, ...)
{
  ...
  case __COMMCTL_IRQ_ENABLE:
    ...
    status = pulBase[REL_Adr_NX51_uartcr / sizeof(cyg_uint8)];

    status |= (MSK_NX51_uartcr_RTIE | MSK_NX51_uartcr_RIE);
    pulBase[REL_Adr_NX51_uartcr / sizeof(cyg_uint8)] = status;
  ...
  case __COMMCTL_IRQ_DISABLE:
    ...
    status = pulBase[REL_Adr_NX51_uartcr / sizeof(cyg_uint8)];
    status &= ~(MSK_NX51_uartcr_RTIE | MSK_NX51_uartcr_TIE | MSK_NX51_uartcr_RIE | MSK_NX51_uartcr_MSIE);
    pulBase[REL_Adr_NX51_uartcr / sizeof(cyg_uint8)] = status;
...
static int cyg_hal_plf_serial_isr(void *__ch_data, int* __ctrlc, CYG_ADDRWORD __vector, CYG_ADDRWORD __data)
{
  ...
  *__ctrlc = 0;
  ...
  if(RX_DATA(status)) 
  {
    c = (cyg_uint8)(pulBase[REL_Adr_NX51_uartdr / sizeof(cyg_uint32)] & 0xFF);
