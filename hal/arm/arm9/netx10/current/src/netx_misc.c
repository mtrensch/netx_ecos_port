...
static NX10_VIC_AREA_T*       s_ptVic      = (NX10_VIC_AREA_T*)NX10_NETX_VIC_AREA;
...
static cyg_uint32 s_aulIntVect[HAL_INT_NUM_VECTORED] =
{
  ...
  CYGNUM_HAL_INTERRUPT_PWM,
  CYGNUM_HAL_INTERRUPT_ENCODER
};
...
void plf_hardware_init(void)
{
  ...
  NX10_POINTER_FIFO_AREA_T*  ptPFifoAreaTmp = (NX10_POINTER_FIFO_AREA_T*)NX10_NETX_PTR_FIFO_AREA;
  ...
  for(uiCnt = 0; uiCnt < HAL_INT_NUM_VECTORED; uiCnt++)
  {
    ...
    s_ptVic->aulVic_vect_cntl[uiCnt] = s_aulIntVect[uiCnt] | MSK_NX10_vic_vect_cntl0_ENABLE;
...
void hal_clock_initialize(cyg_uint32 period)
{
  ...
  s_ptArmTimer->ulArm_timer_irq_msk_reset     = MSK_NX10_arm_timer_irq_msk_reset_timer0_irq;
  ...
  // continous mode
  s_ptArmTimer->aulArm_timer_config_timer[0] = 1 << SRT_NX10_arm_timer_config_timer0_mode;

  // Interrupt supported:
  // First of all acknowledge an optional present interrupt
  s_ptArmTimer->ulArm_timer_irq_raw = MSK_NX10_arm_timer_irq_raw_timer0_irq;

  // Enable the Interrupt in the specific Timer register
  s_ptArmTimer->ulArm_timer_irq_msk_set = MSK_NX10_arm_timer_irq_msk_set_timer0_irq;
